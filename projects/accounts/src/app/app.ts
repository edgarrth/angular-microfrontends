...
interface Account { id: string; alias: string; type: string; currency: string; availableBalance: number; status: string; }
interface Card { id: string; accountId: string; brand: string; maskedNumber: string; status: string; }
interface Beneficiary { id: string; alias: string; bankName: string; accountNumber: string; currency: string; }
...
export class App {
  ...
  readonly beneficiaries$: Observable<Beneficiary[]> = this.http
    .get<ApiResult<Beneficiary[]>>(`${this.config.accountsApiUrl}/beneficiaries`)
    .pipe(map((result) => result.data), shareReplay({ bufferSize: 1, refCount: true }));
  ...
  registerBeneficiary(): void {
    this.http
      ...
      .subscribe((result) => alert(`Beneficiario registrado: ${result.data.alias}`));
  }
}
